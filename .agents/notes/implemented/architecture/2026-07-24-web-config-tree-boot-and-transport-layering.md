...
**Composition is one flat assembled tree.** The original Web tree held every Host, gateway, WebServer, and `dsh.client` row in one overlay. That stance later became repository-wide profile bundles: shared core rows moved into `dsh-base`, and the [shared GUI composition](2026-08-16-shared-gui-composition-and-explicit-carrier.md) now owns transport-neutral Host and client rows in `dsh-gui-app`, with `dsh-web-app` reduced to browser transport rows. Every plugin remains one patchable row and every config field remains yml-editable. The `dsh-client-hmr` row is an ordinary always-on Web bundle row (originally appended in code by `--dev`; the flag is retired). Row order carries no load semantics; activation is service-availability driven. The shared audit rejects imports with no fiber, awaits only failed fibers to recover original activation errors, and reports services that leave a fiber `PENDING`; before throwing, it marks those exact rejection reasons through one process checkpoint so `installFailLoud` coalesces Loader's duplicate notification while unrelated unhandled rejections remain fatal. The Node app-boot artifact embeds `@cordisjs/plugin-include` while leaving `@cordisjs/plugin-loader` external, so the include's `EntryTree` and the host bind to one Loader peer instead of splitting a config tree across two Loader implementations.
...
**The transport splits five ways.** `dsh-host-apiproxy` is the gateway plugin (`api-gateway` row): it default-exports `ApiProxyService`, configures only `{nativeOpen?}`, consumes the base layer's entry-point-neutral `ctx.agentDefaultModel`, provides `ctx.apiProxy`, remains transport-agnostic, and registers no routes. `dsh-host-webserver` is a plain route-registration plugin: `WebServer` provides `ctx.webServer` (`register(route) → disposer` with duplicate-pattern throw, `tapIndex` transforms applied in registration order, `port`), listens on activation, answers per-request failures with 400 and logging, and knows no harness concepts. The connection Host half owns transport-neutral API dispatch plus Web and IPC adapters; its Web entry binds `/api` and WebSocket upgrades. `ClientModuleRegistry` owns incremental package scanning, strict resource manifests, and `onRebuilt`/`onGraphChanged` notification, while its `/web` entry alone owns the bundle route and index tap. The HMR Host half owns Web development reload through `fs.watchFile` membership and the `/plugins/events` SSE route.
...
- Recomposing a Web deployment is a bundle/profile patch edit; shared GUI rows have one owner in `dsh-gui-app`, while the retired pieces (`mountWebPlugins`, `CLIENT_PACKAGES`, `createHostWebPluginRegistry`, `startWebServer`, the WebServer's graph/SSE/API knowledge) remain deleted.
