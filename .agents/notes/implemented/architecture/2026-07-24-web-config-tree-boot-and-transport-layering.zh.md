...
**组合结果是一棵平铺配置树。** 最初的 Web 配置树在一份 overlay 中持有全部 Host、网关、WebServer 与 `dsh.client` 行。这一立场后来演进为全仓 profile 组合包：共享 core 行进入 `dsh-base`，[共享 GUI 组合](2026-08-16-shared-gui-composition-and-explicit-carrier.md)现在通过 `dsh-gui-app` 持有与传输无关的 Host 和客户端行，`dsh-web-app` 则收窄为浏览器传输行。每个插件仍是一条可 patch 的配置行，每个 config 字段仍可通过 yml 修改。`dsh-client-hmr` 行是普通且始终启用的 Web 组合包行（最初由 `--dev` 在代码中追加；该旗标已废除）。行序无装载语义；激活由服务可用性驱动。共享 audit 会拒绝没有 fiber 的 import、仅等待失败的 fiber 以恢复原始激活错误，并报告让 fiber 停在 `PENDING` 的服务；抛出错误前，审计会通过一个进程级检查点标记这些 rejection 的确切原因，从而让 `installFailLoud` 将 Loader 的重复通知合并为一次，而无关的未处理 rejection 仍然致命。Node app-boot 产物内嵌 `@cordisjs/plugin-include`，但将 `@cordisjs/plugin-loader` 保持为外部依赖，因此 include 的 `EntryTree` 与 Host 会绑定到同一个 Loader peer，而不会让一棵配置树横跨两个 Loader 实现。
...
**传输五分。** `dsh-host-apiproxy` 是网关插件（`api-gateway` 行）：默认导出 `ApiProxyService`，只配置 `{nativeOpen?}`，消费 base 层不偏向特定入口的 `ctx.agentDefaultModel`，provide `ctx.apiProxy`，保持传输无关且不注册路由。`dsh-host-webserver` 是朴素的路由注册插件：`WebServer` provide `ctx.webServer`（`register(route) → disposer`、重复 pattern 即抛、`tapIndex` 按注册序应用、`port`），激活即 listen，单请求失败时答 400 并记日志，且不认识任何 harness 概念。connection Host half 持有与传输无关的 API dispatch 及 Web／IPC adapter；其 Web 入口绑定 `/api` 与 WebSocket upgrade。`ClientModuleRegistry` 持有单包增量扫描、严格资源 manifest 和 `onRebuilt`／`onGraphChanged` 通知，只有它的 `/web` 入口持有 bundle route 与 index tap。HMR（热模块替换）Host half 通过 `fs.watchFile` membership 与 `/plugins/events` SSE route 持有 Web 开发期重载。
...
- 重组一个 Web 部署就是修改组合包／profile patch；共享 GUI 行由 `dsh-gui-app` 单独持有，退役件（`mountWebPlugins`、`CLIENT_PACKAGES`、`createHostWebPluginRegistry`、`startWebServer`、WebServer 的图／SSE／API 知识）仍保持删除。
