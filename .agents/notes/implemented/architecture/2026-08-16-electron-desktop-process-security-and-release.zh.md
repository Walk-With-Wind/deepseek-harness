...
在 Electron 中运行共享 GUI 会引入 Web 产品不具备的权限：原生窗口与对话框、已安装包文件系统、OS 进程控制、签名和更新 API，以及能通过显式选择的 home 与 CLI／Web 进程共享持久数据的本地 Host。若把 Harness Host 放进 Main，Electron 要求保持响应的进程就会同时持有不可信 Renderer 输入、原生 UI 策略、插件、存储和长时间 Agent 工作。暴露通用 Preload bridge 或启动私有 HTTP server 会重新建立宽泛远程控制接口，并削弱无 listener 目标。若把进程退出视为干净关停，更新或崩溃还会与 session flush 和租约释放竞态。
...
- [GUI 分层／RPC Note](2026-07-19-gui-layering-and-rpc-protocol.md)、[Web 传输 Note](2026-07-24-web-config-tree-boot-and-transport-layering.md)和[共享 GUI carrier Note](2026-08-16-shared-gui-composition-and-explicit-carrier.md)仍保持活跃：本决策实现其扩展点，但不替代协议、Web 或共享组合理由。在原生证据尚未完整时，[Community 发行提案](../../proposed/architecture/2026-08-22-community-desktop-release.md)负责 fork 专属身份、发布与启用标准。
