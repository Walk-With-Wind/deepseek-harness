...
浏览器无法生成持久引用，因此 GUI 图片路径接受可重新打开的原始字节源，而不是规范 `ContentBlock[]`：
...
生产客户端先发送有界 JSON Manifest，再发送原始图片字节，并且每次提交尝试都重新打开 `File.stream()`。Web 载体仍会在分派前缓冲物理 `node:http` 正文，Desktop IPC 则每次最多拉取一个 1 MiB 分片；两条路径都不会通过 JSON/base64 或数值数组复制图片批次。一元 JSON `session.prompt` 仍供类型化进程内调用方使用，但组装后的 GUI 图片接收只使用二进制路由。

宿主会校验图片数量、总字节数、单张图片字节数、声明的 MIME 与完整解码后的光栅图片是否一致、固有尺寸和解码像素数。`prepareImages` 按提交顺序消费精确长度的来源，本地提供方每次只把一张图片暂存到磁盘，同一次完整解码会生成最长边 480px 的有界 WebP 派生图。完整批次通过准入且物理请求证明不存在尾随字节前，不发布任何内容寻址对象。如果后续发布 I/O 操作失败，宿主不会追加用户事件，但先前的不可变内容寻址对象可能保持无引用状态；第一版将清理留给未来按引用感知的垃圾回收，而不向去重存储添加破坏性回滚。只有每张图片都成功后，宿主才会用规范化文本和按提交顺序排列的持久图片块调用 agent。失败时不公开任何附件路径或原始字节。Utility 持有的本地提供方会关闭 libvips 缓存，避免已完成批次留下与既往附件总量成比例的原生内存。

附件字节路由只读且限定于会话作用域。只有该会话中的持久事件引用了所请求的附件标识符，宿主才提供字节。缩略图请求使用只供界面的 WebP 派生图；在预览功能引入前写入的对象则按需生成。原图请求读取规范的内容寻址对象。会话处于渲染状态时，客户端按会话、附件标识符与用途对加载操作去重，并将缩略图工作串行化；已渲染会话释放时会撤销已解析的 URL，并在分配对象 URL 前拒绝已失效的延迟加载，以免已卸载的会话或已释放的服务重新写入缓存。
...
历史记录折叠会在用户消息和助手消息中保留 `ImageBlock`。用户图片在文本上方靠尾端对齐；助手图片则保留在靠前端叙述流中的原内容块位置。`MessageImage` 根据记录的尺寸派生稳定的行内边界框，只有进入视口纵向提前 256px 的范围后才请求缩略图，使用有界 `object-fit: cover` 呈现，并将对象缺失或损坏转换为可重试的错误控件。输入区附件栏图片只有接近横向视口时才绑定 Blob URL；两个界面都要求浏览器延迟、异步解码。

输入区缩略图和每个 `MessageImage` 各自持有临时预览状态，并调用同一个纯 `ImageLightbox`。历史灯箱会立即用已就绪的缩略图打开，再请求并替换为原图；原图读取失败时保留缩略图，下次打开重新尝试。模态框只限制显示尺寸；它会聚焦关闭控件，并在关闭时把焦点恢复到先前的目标。

### 限制与信任边界

第一版仅接受 PNG、JPEG、WebP 和 GIF。不接受 SVG 和远程 URL。默认限制为每张图片 5 MiB、每条消息 20 张图片和 100 MiB 图片总字节数，以及每张图片 4,000 万个固有像素。这些随部署变化的限制属于经过校验的后端配置，并由宿主在持久化前强制执行。Web 连接适配器为每个 API 请求设置独立且可配置的 `maxRequestBodyBytes` 上限（默认 160 MiB）；如果该上限无法容纳附件服务的原始图片总量限制、固定长度前缀与有界 Manifest，加载就会失败。其 `node:http` bridge 会在未声明长度的正文越过上限时立即拒绝，而不是继续读完。Desktop 累计执行同一请求上限，同时只保留有界的在途 IPC 分片。

格式错误的 Manifest、声明长度与实际长度不一致、尾随字节、不支持或不匹配的媒体、截断的图片数据、超出字节限制、超出图片数量、超出像素限制、对象缺失和完整性不匹配都会返回稳定的结构化错误。原始文件名只保留用于显示的末段，控制字符会被移除，并且任何本地路径都不会写入日志或返回浏览器。
...
| `packages/attachment/attachment-local` | 私有内容寻址存储、磁盘暂存、有界预览派生图、完整光栅解码、完整性校验和配置。 |
...
| `packages/client/connection` 和 `packages/client/runtime` | Web 请求上限、Desktop IPC 流式传输、二进制 Prompt 上传、按用途读取附件、fixture（测试前置数据）图片和持久引用折叠。 |
| `packages/client/ui-attachment` 和 `packages/client/ui-conversation` | 每个会话的草稿图片、按视口延迟加载的附件栏与历史缩略图、按用途管理 URL 生命周期和按需原图预览。 |
...
已实现的范围包括附件 seam、角色无关的图片块、Pi-AI 输入转换、DeepSeek 拒绝、宿主持久化顺序、Web／Desktop 原始二进制上传与读取协议、当前图片限制执行、磁盘暂存与预览派生、大小受限的 Web 请求体和 Desktop IPC 分片、内存草稿图片、按视口延迟加载的粘贴／拖放附件栏、用户与助手历史缩略图、按需原图预览、压缩处理，以及组装后无需密钥的 Web 覆盖。
...
- 存储测试覆盖精确长度的流式准入、整批发布前全量通过、内容寻址去重、私有权限、有界预览生成、旧对象回退、对象损坏或缺失时的失败，以及收紧部署限制后读取历史数据。
- 宿主与协议测试覆盖先持久化再追加事件的顺序、日志中不含 base64 且产品路径使用二进制、会话作用域授权、缩略图／原图选择、能力拒绝、上传限制、有界 Web 正文与 IPC 分片、小分片合并、图片准入与模型选择的竞态（排队与 steering 两种放置）、待发布状态、未发布即空闲时的门槛释放、仅文本的队列编辑，以及压缩后依据当前派生历史进行的选择。
- 客户端单元测试覆盖粘贴与拖放、混合剪贴板文本、仅图片发送、每次重试使用新流、草稿恢复、顺序、附件栏／历史图片延迟激活、按用途执行草稿／会话作用域／应用层级对象 URL 清理，以及释放后才完成的延迟读取；keyless 的组装后构建产物通道（`apps/web/tests/image-display.snapshot.ts`，`DSH_EXAMPLE_MODE=lib pnpm run test:snapshot`）覆盖经授权附件路由渲染的历史用户与助手图片画廊、原图 lightbox，以及 composer 粘贴缩略图条。已安装 Desktop 验收通过生产载体提交 20 张各 5 MiB 的图片，并把三个进程的实测峰值 RSS 增量保持在 300 MiB 以下。
...
- Web `node:http` bridge 在 Fetch 分派前仍会持有一个完整的原始上传请求。Desktop IPC 使用一个有界在途分片流式传输同一正文；若要进一步降低 Web 驻留内存，需要流式 HTTP bridge。
- 未发送图片无法跨重载保留。持久草稿需要配额和遗留对象清理，而不是隐式复用消息存储。
- 预览派生图会在规范对象旁增加有界磁盘用量。只有明确打开灯箱后才解码原图；像素限制、一次只打开一个预览、缩略图串行处理和对象 URL 释放可以约束但无法消除浏览器瞬时内存占用。
