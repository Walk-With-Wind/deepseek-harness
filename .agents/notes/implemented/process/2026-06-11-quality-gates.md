...
- knip (dead code/deps), publint (package correctness), workspace constraints (workspace rules: private, cordis peer+dev, uniform version, ESM), and a NodeNext consumer typecheck for built package declarations. Knip treats repository sources as authoritative even when `lib/` exists: its wrapper excludes generated files and removes only a workspace's `zod` ignore when `lib/typert.host.js` exposes that generated dependency, so clean and built trees enforce the same strict configuration hints without treating artifacts as owned source.
