...
Desktop release builds are native per target: macOS arm64, macOS x64, and Windows x64. macOS investigation builds use an ad-hoc signature so that the native updater can read the application bundle identity after fuse changes; this signature does not establish a distribution identity. With `DSH_DESKTOP_SIGNING=1`, macOS uses Developer ID signing and notarization, while Windows uses trusted Authenticode signing. The protected CI environment supplies the platform credentials, and `DSH_DESKTOP_REQUIRE_SIGNING=1` makes artifact verification reject products without the required release signature. The [release cookbook](../../docs/cookbook/releasing-desktop.md) owns credential names and publication order.
...
Shutdown first stops new work, asks Utility to flush and dispose, waits for `host/quiescent`, and only then exits or installs an update. A grace timeout escalates termination without claiming quiescence. macOS and Windows updates use Electron's native updater against the compiled HTTPS origin.
