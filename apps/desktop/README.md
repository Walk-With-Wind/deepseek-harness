...
Windows handles Squirrel install, update, uninstall, and obsolete events before constructing the regular Host. Shortcut maintenance runs in a detached updater process, while the application exits after one second without waiting for that process to close; this releases the parent Squirrel operation even when both updater processes need the same lock. Installer acceptance limits each Squirrel uninstall to 30 seconds and reports a timeout before runner cleanup.

Windows installed-data acceptance determines the measured process set when an attachment or export phase starts, then reads those stable PIDs' working sets once per second instead of enumerating the WMI process tree for every sample. Each PowerShell query has a 10-second limit. The acceptance also emits launch, attachment-persistence, optional export/endurance, and shutdown phase markers so a native runner's stopping point is visible in the live log.

The final installed-directory native probe loads every `.node` file and executes Sharp, Koffi, ripgrep, and node-pty. It exits explicitly after flushing its successful JSON because Windows native modules can retain libuv handles unrelated to the acceptance result. The installer driver emits `install`, `smoke`, and `uninstall` phases for both the initial installation and reinstall, distinguishing a completed child that did not exit from a Squirrel maintenance failure.
