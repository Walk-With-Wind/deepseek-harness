...
DeepSeek Harness Desktop 是共享 GUI 组合的 Electron 产品外壳。Main 持有原生策略、单个沙箱 BrowserWindow、`app://localhost` 协议、更新编排、对话框、诊断和 Utility supervisor；Preload 只暴露经过校验的 Renderer 协议；Utility 持有 Harness Host 与 home 级 Host 租约；Renderer 通过 IPC `ClientCarrier` 运行共享 `AppGuiEntry`，无法访问 Node 或 Electron，也不会打开本机网络监听端口。

应用与 CLI 和 Web UI 使用同一份 `DSH_HOME`、设置、凭据、会话与工作区。Host 租约保证这些产品针对同一规范化 home 只能有一个 Host 写入方。第二个 Desktop 实例会聚焦首个窗口；其他产品若未取得租约，会在挂载业务插件前失败。
