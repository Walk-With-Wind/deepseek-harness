...
import {
  ...
  assertDesktopReleaseDownloadUrl,
...
export function desktopUpdateFeedUrl(
  ...
  return `${DESKTOP_UPDATE_BASE_URL}/${channel}/${platform}-${arch}/${suffix}`
}
...
export class ElectronNativeUpdateAdapter implements NativeUpdateAdapter {
  ...
  constructor(
    ...
    private readonly readFeed: (url: string) => Promise<string> = readUpdateFeed,
  ) {
    updater.setFeedURL({
      url: this.feedUrl,
  ...
  private async checkFeed(): Promise<void> {
    let source: string
    try {
      source = await this.readFeed(this.feedUrl)
    } catch {
      this.emit({ type: 'error', code: 'UPDATE_NETWORK_UNAVAILABLE' })
      return
    }
    try {
      if (this.platform === 'darwin') validateMacReleaseFeed(source)
      else validateWindowsReleaseIndex(source)
    } catch {
      this.emit({ type: 'error', code: 'UPDATE_METADATA_INVALID', retryable: false })
      return
    }
    if (!this.disposed) this.updater.checkForUpdates()
  }
}

function validateMacReleaseFeed(value: string): void {
  const parsed = JSON.parse(value) as Record<string, unknown>
  if (typeof parsed.url !== 'string' || typeof parsed.name !== 'string'
    || !/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(parsed.name)) {
    throw new Error('macOS release feed 格式无效')
  }
  assertDesktopReleaseDownloadUrl(parsed.url, parsed.name)
  if (!new URL(parsed.url).pathname.toLowerCase().endsWith('.zip')) {
    throw new Error('macOS release feed 资产类型无效')
  }
}
...
async function readUpdateFeed(url: string): Promise<string> {
  const response = await fetch(url, { redirect: 'error' })
  if (!response.ok) throw new Error(`Desktop update feed HTTP ${String(response.status)}`)
  return await response.text()
}

function validateWindowsReleaseIndex(value: string): void {
  const lines = value.replace(/^\uFEFF/, '').split(/\r?\n/).filter(line => line !== '')
  if (lines.length === 0) throw new Error('Windows RELEASES 为空')
  for (const line of lines) {
    const match = /^[a-f0-9]{40}\s+(\S+)\s+(\d+)(?:\s+#.*)?$/i.exec(line)
    const downloadUrl = match?.[1]
    const size = match?.[2]
    if (downloadUrl === undefined || size === undefined || !/^\d+$/.test(size)) {
      throw new Error('Windows RELEASES 格式无效')
    }
    assertDesktopReleaseDownloadUrl(downloadUrl)
    if (!new URL(downloadUrl).pathname.toLowerCase().endsWith('.nupkg')) {
      throw new Error('Windows RELEASES 资产类型无效')
    }
  }
}
