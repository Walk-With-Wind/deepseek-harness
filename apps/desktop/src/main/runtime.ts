...
export class DesktopMainRuntime {
  ...
  constructor(
    ...
    this.logger = new DesktopJsonlLogger(
      this.home, 'main', app.getVersion(), this.config.logMaxBytes, this.config.logMaxFiles,
    )
    this.rendererCrashLogger = new DesktopJsonlLogger(
      this.home, 'renderer-crash', app.getVersion(), this.config.logMaxBytes, this.config.logMaxFiles,
    )
  ...
  private spawnUtility(generation: number): void {
    ...
    const child = utilityProcess.fork(this.utilityPath, [], {
      serviceName: 'DeepSeek Harness Utility Host',
    ...
    child.on('spawn', () => {
      ...
      const home = canonicalizeHostHome(resolveDshHome())
  ...
  private armUpdateTimer(initial: boolean): void {
    ...
    const delay = initial
      ? this.config.updateInitialDelayMs + jitter
      : this.config.updateCheckIntervalMs + jitter
    this.updateTimer = setTimeout(() => {
      this.updateTimer = undefined
      this.updateProvider.check()
      this.armUpdateTimer(false)
    }, delay)
  }
