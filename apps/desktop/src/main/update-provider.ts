...
export interface NativeUpdateAdapter {
  ...
  check(): void
  /** 已下载后立即退出并完成更新；否则 Electron 会在下次启动时应用。 */
  quitAndInstall(): void
  /** 移除平台事件监听器。 */
  dispose?(): void
}
...
/** Main 拥有的更新 provider；手动检查授权自动下载，用户另行选择是否立即重启。 */
export class DesktopUpdateProvider {
  ...
  constructor(private readonly options: DesktopUpdateProviderOptions) {
    ...
    if (options.updatesEnabled && options.native === undefined) {
      throw new Error('启用 Desktop 更新时缺少原生更新适配器')
    }
    this.unsubscribeNative = options.updatesEnabled
      ? options.native?.subscribe((event) => { this.receive(event) })
      : undefined
  }
  ...
  /** 用户发起检查并授权平台下载可用更新；并发检查被拒绝。 */
  check(): boolean {
    if (this.disposed || !this.options.updatesEnabled) return false
  ...
  /** 标记用户选择立即重启完成更新；实际调用必须等宿主 quiescent。 */
  ...
  private receive(event: NativeUpdateEvent): void {
    ...
    switch (event.type) {
      ...
      case 'error':
        this.fail(event.code, event.retryable ?? true)
        return
      default:
  ...
  private validateTarget(version: string, updateUrl: string | undefined): ParsedVersion | undefined {
    ...
    try {
      ...
      if (this.channel === 'canary'
        && target.prerelease.length === 0
        && target.core.some((part, index) => part !== this.current.core[index])) {
        throw new Error('canary 只接受同版本线 final 晋级')
      }
      if (this.options.platform === 'darwin' && (updateUrl === undefined || updateUrl === '')) {
        throw new Error('macOS 更新缺少 Release 资产地址')
      }
      if (updateUrl !== undefined && updateUrl !== '') {
        assertDesktopReleaseDownloadUrl(updateUrl, target.raw)
      }
      return target
    } catch {
  ...
  private nativeAdapter(): NativeUpdateAdapter {
    if (this.options.native === undefined) throw new Error('Desktop 原生更新适配器不可用')
