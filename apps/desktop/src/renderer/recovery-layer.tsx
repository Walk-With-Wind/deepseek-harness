  ...
  return (
    <section className={styles.layer} aria-live="polite" aria-busy={!isFailure}>
      <div className={styles.panel}>
        <p className={styles.eyebrow}>{DESKTOP_PRODUCT_NAME}</p>
  ...
  const content = state.phase === 'CHECKING'
    ...
    : state.phase === 'DOWNLOADING'
      ? {
        ...
        detail: '手动检查已授权平台下载；下载完成后会在下次正常启动时应用。',
      }
      : state.phase === 'READY'
        ? { title: `${state.targetVersion} 已准备就绪`, detail: '更新会在下次正常启动时应用；也可先安全排空本地 Host 后立即重启。' }
        : state.phase === 'INSTALLING'
          ? { title: '正在安全退出并完成更新', detail: '正在等待会话、日志与本地资源完全静止。' }
