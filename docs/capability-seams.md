...
| `ctx.clientModules` | `core` | `modules` | - | `hmr` | - | Composes the __DSH_BOOT__ graph and strict resource manifest from an incremental dsh.client scan; product adapters expose those resources, while HMR consumes rebuild notifications. |
| `ctx.hostLease` | `core` | [`app-boot`](../packages/boot/app-boot) | - | - | - | Canonicalizes one Harness home, proves exclusive cross-product ownership with a platform endpoint, and releases that endpoint under the root Cordis lifecycle. |
