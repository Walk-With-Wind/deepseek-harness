...
flowchart LR
  ...
  pkg_modules["modules"]
  pkg_hmr["hmr"]
  svc_clientModules["ctx.clientModules<br/>GUI client module registry"]
  pkg_app_boot["app-boot"]
  svc_hostLease["ctx.hostLease<br/>Exclusive Host-home owner"]
...
| `ctx.clientModules` | `core` | `modules` | - | `hmr` | - | 通过增量 `dsh.client` 扫描组合 __DSH_BOOT__ 图与严格资源 manifest；产品 adapter 发布这些资源，HMR 消费重建通知。 |
| `ctx.hostLease` | `core` | [`app-boot`](../packages/boot/app-boot) | - | - | - | 规范化一个 Harness home，通过平台 endpoint 证明跨产品独占所有权，并在根 Cordis 生命周期下释放该 endpoint。 |
