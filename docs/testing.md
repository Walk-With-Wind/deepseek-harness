...
- **Snapshot** (`pnpm run test:snapshot`): keyless outputs cover transport and presentation; persisted logs pin assembled backend behavior. ACP replays a recorded session through the real automation-server example and diffs normalized JSON-RPC plus the re-persisted log ([ACP snapshot Agent Note](../.agents/notes/implemented/testing/2026-06-19-acp-snapshot-tests.md)); headless scenarios boot explicit examples through an unexported JSONL driver, while `apps/cli` owns product `dsh --profile headless` acceptance. Use `test:snapshot:record` for model transcript changes and `test:snapshot:refresh` when replay input remains valid; review every diff. The ACP `text-turn` scenario pins full system-prompt/tool-schema content; others tokenize it so an edit churns one line ([pinned-header Agent Note](../.agents/notes/archived/testing/2026-07-06-pin-request-header-content-in-one-scenario.md)).
...
- **Desktop**: source, shared-GUI snapshot, packaged-process, artifact, and protected native-matrix evidence follow the [Desktop release runbook](cookbook/releasing-desktop.md).
