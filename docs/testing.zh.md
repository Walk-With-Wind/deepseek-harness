...
- **快照**（`pnpm run test:snapshot`）：无密钥输出覆盖传输与呈现，持久化日志固定组装后的后端行为。ACP 通过真实自动化服务器示例回放录制会话，并对归一化 JSON-RPC 与重新持久化的日志执行 diff（[ACP 快照 Agent Note](../.agents/notes/implemented/testing/2026-06-19-acp-snapshot-tests.md)）；headless 场景通过未导出的 JSONL driver 启动显式示例，`apps/cli` 负责产品 `dsh --profile headless` 验收。模型文本记录变化时使用 `test:snapshot:record`，回放输入仍有效时使用 `test:snapshot:refresh`；审查每处差异。ACP `text-turn` 场景固定完整系统提示词与工具 schema；其他场景将其 token 化，因此修改只扰动一行（[pinned-header Agent Note](../.agents/notes/archived/testing/2026-07-06-pin-request-header-content-in-one-scenario.md)）。
...
- **Desktop**：源码、共享 GUI 快照、打包进程、产物与受保护原生矩阵证据遵循 [Desktop 发行手册](cookbook/releasing-desktop.md)。
