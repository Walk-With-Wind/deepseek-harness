...
- Your patch can override rows from earlier layers by `id` — the same way [the `dsh-gui-app` bundle](../../../../packages/bundle/gui-app/cordis.patch.yml) overrides `dsh-base` rows — but must restate every key the row needs, not just the changed one.
