...
function smokeMacDmg() {
  ...
  const installed = join(temporary, 'Applications', `${DESKTOP_PRODUCT_NAME}.app`)
...
function smokeWindowsSquirrel() {
  const setup = findArtifact(DESKTOP_WINDOWS_SETUP_EXE)
  const installRoot = join(process.env.LOCALAPPDATA ?? '', DESKTOP_WINDOWS_SQUIRREL_PACKAGE_ID)
  if (installRoot === DESKTOP_WINDOWS_SQUIRREL_PACKAGE_ID) {
