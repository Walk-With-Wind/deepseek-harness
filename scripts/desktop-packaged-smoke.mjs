...
const product = process.env.DSH_DESKTOP_SMOKE_PRODUCT === undefined
  ? join(root, '.artifacts', 'desktop', 'out', `DeepSeek Harness-${platform}-${arch}`)
  : resolve(process.env.DSH_DESKTOP_SMOKE_PRODUCT)
const executable = process.env.DSH_DESKTOP_SMOKE_EXECUTABLE === undefined
  ? platform === 'darwin'
    ? join(product, 'DeepSeek Harness.app', 'Contents', 'MacOS', 'deepseek-harness')
    : join(product, platform === 'win32' ? 'deepseek-harness.exe' : 'deepseek-harness')
