...
import { resetDesktopForgeOutput } from './lib/desktop-forge-output.ts'
import {
...
const packageDiagnosticsPreload = resolve(root, 'scripts/desktop-package-diagnostics-preload.ts')
...
type DesktopReleaseMode = 'development' | 'unsigned-preview' | 'signed'

function desktopReleaseMode(environment: NodeJS.ProcessEnv): DesktopReleaseMode {
  const value = environment.DSH_DESKTOP_RELEASE_MODE
    ?? (environment.DSH_DESKTOP_SIGNING === '1' ? 'signed' : 'development')
  if (value !== 'development' && value !== 'unsigned-preview' && value !== 'signed') {
    throw new Error(`desktop: DSH_DESKTOP_RELEASE_MODE 无效 ${value}`)
  }
  if ((value === 'signed') !== (environment.DSH_DESKTOP_SIGNING === '1')) {
    throw new Error('desktop: signed 发行模式必须与 DSH_DESKTOP_SIGNING=1 一致')
  }
  return value
}
...
async function prepareStaging(
  ...
  const releaseMode = desktopReleaseMode(process.env)
      ...
      env: {
        ...
        ...(diagnosticsEnabled
          ? {
            ...
            DSH_DESKTOP_DIAGNOSTICS_FORGE_CLI: forgeCli,
          }
          : {}),
      },
    })
