...
describe('Knip source-plane configuration', () => {
  ...
  it('仅在构建产物存在时忽略 lib 并撤销对应的 zod 源码豁免', () => {
    ...
    const resolved = resolveKnipConfigForArtifacts(config, {
      ...
      artifactExists: path => !path.includes('interaction/commands'),
    })

    expect(resolved.ignore).toEqual(['retained-pattern', '**/lib/**'])
    expect(resolved.workspaces?.['packages/context/file-reference']?.ignoreDependencies).toBeUndefined()
    expect(resolved.workspaces?.['packages/context/session-reference']?.ignoreDependencies).toBeUndefined()
