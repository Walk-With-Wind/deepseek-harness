...
export function desktopArtifactPaths(platform: NodeJS.Platform, arch: string): DesktopArtifactPaths {
  const product = resolve(root, '.artifacts/desktop/out', `DeepSeek Harness-${platform}-${arch}`)
  if (platform === 'darwin') {
    const app = join(product, 'DeepSeek Harness.app')
    return {
      app,
  ...
  return {
    app: product,
    executable: join(product, platform === 'win32' ? 'deepseek-harness.exe' : 'deepseek-harness'),
