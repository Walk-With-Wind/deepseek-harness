...
describe('Desktop release materials', () => {
  ...
  it('生成可重验的 SBOM、notices、hash 与 provenance', async () => {
    ...
    const provenance = JSON.parse(readFileSync(join(paths.output, 'build-provenance.json'), 'utf8'))
    expect(provenance.build).toMatchObject({
  ...
  it('拒绝 staging 中的官方或第三方发行身份', async () => {
    const paths = setup()
    const buildInfoPath = join(paths.staging, 'build-info.json')
    const buildInfo = JSON.parse(readFileSync(buildInfoPath, 'utf8'))
    writeFileSync(buildInfoPath, JSON.stringify({ ...buildInfo, publisher: 'DeepSeek AI' }))
    await expect(generateDesktopReleaseMaterials({
      root: paths.root, staging: paths.staging,
      artifactRoot: paths.artifacts, outputRoot: paths.output,
    })).rejects.toThrow(/发行身份/)
  })
})
