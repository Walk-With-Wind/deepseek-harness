...
export async function generateDesktopReleaseMaterials(options: DesktopReleaseMaterialOptions): Promise<void> {
  ...
  assertCommunityBuildInfo(buildInfo)
...
function assertCommunityBuildInfo(buildInfo: Record<string, unknown>): void {
  if (buildInfo.distribution !== DESKTOP_DISTRIBUTION
    || buildInfo.repository !== DESKTOP_REPOSITORY
    || buildInfo.applicationId !== DESKTOP_APPLICATION_ID
    || buildInfo.publisher !== DESKTOP_PUBLISHER) {
    throw new Error('desktop-materials: build-info 发行身份无效')
  }
  if (typeof buildInfo.version !== 'string'
    || !/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(buildInfo.version)
    || typeof buildInfo.sourceCommit !== 'string'
    || !/^[a-f0-9]{40}$/.test(buildInfo.sourceCommit)
    || typeof buildInfo.sourceDate !== 'string'
    || !Number.isFinite(Date.parse(buildInfo.sourceDate))
    || typeof buildInfo.platform !== 'string'
    || typeof buildInfo.arch !== 'string') {
    throw new Error('desktop-materials: build-info provenance 无效')
  }
  assertDesktopReleasePlatform(buildInfo.platform)
  if (buildInfo.arch !== 'arm64' && buildInfo.arch !== 'x64') {
    throw new Error(`desktop-materials: build-info 架构无效 ${buildInfo.arch}`)
  }
}
...
function renderCycloneDx(
  ...
  return {
    ...
    metadata: {
      ...
      properties: [
        { name: `${DESKTOP_APPLICATION_ID}:sourceCommit`, value: String(buildInfo.sourceCommit ?? 'unknown') },
        { name: `${DESKTOP_APPLICATION_ID}:stagingSha256`, value: closureDigest },
        { name: `${DESKTOP_APPLICATION_ID}:platform`, value: String(buildInfo.platform ?? 'unknown') },
        { name: `${DESKTOP_APPLICATION_ID}:arch`, value: String(buildInfo.arch ?? 'unknown') },
      ],
    },
    components,
  }
