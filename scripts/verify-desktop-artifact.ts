...
function verifyMacBundle(
  app: string,
  asar: string,
  asarModule: DesktopAsarModule,
  nativeVersions: { readonly appVersion: string; readonly buildVersion: string },
): void {
  ...
  if (plist.CFBundleShortVersionString !== nativeVersions.appVersion
    || plist.CFBundleVersion !== nativeVersions.buildVersion) {
    throw new Error('desktop-artifact: Info.plist 原生版本与产品版本映射不一致')
  }
...
function verifyMacSignature(app: string): void {
  ...
  if (!strict) return
  const expectedTeamId = requiredReleaseIdentity('DSH_MAC_EXPECTED_TEAM_ID')
  if (!/^[A-Z0-9]{10}$/.test(expectedTeamId)) {
    throw new Error('desktop-artifact: DSH_MAC_EXPECTED_TEAM_ID 不是有效 Apple Team ID')
  }
  const expectedAuthority = requiredReleaseIdentity('DSH_MAC_SIGN_IDENTITY')
  ...
  if (detail.status !== 0) throw new Error(`desktop-artifact: 无法读取 macOS 签名身份：${detail.stderr.trim()}`)
  if (/Signature=adhoc/.test(detail.stderr)) throw new Error('desktop-artifact: release 产物不得使用 ad-hoc 签名')
  const lines = detail.stderr.split(/\r?\n/)
  if (!lines.includes(`TeamIdentifier=${expectedTeamId}`)) {
    throw new Error('desktop-artifact: macOS 签名 TeamIdentifier 与保护环境不一致')
  }
  if (!lines.includes(`Authority=${expectedAuthority}`)) {
    throw new Error('desktop-artifact: macOS 签名 Authority 与保护环境不一致')
  }
...
function verifyWindowsSignature(executable: string): void {
  ...
  const expectedThumbprint = requiredReleaseIdentity('DSH_WINDOWS_EXPECTED_SIGNER_THUMBPRINT')
    .replaceAll(' ', '').toUpperCase()
  if (!/^[0-9A-F]{40}$/.test(expectedThumbprint)) {
    throw new Error('desktop-artifact: DSH_WINDOWS_EXPECTED_SIGNER_THUMBPRINT 不是有效 SHA-1 指纹')
  }
  const script = [
    ...
    'if ($null -eq $signature.SignerCertificate) { throw "Desktop executable signer certificate is missing" }',
    '$actual = $signature.SignerCertificate.Thumbprint.Replace(" ", "").ToUpperInvariant()',
    'if ($actual -ne $env:DSH_WINDOWS_EXPECTED_SIGNER_THUMBPRINT) { throw "Desktop executable signer is unexpected" }',
    'if ($null -eq $signature.TimeStamperCertificate) { throw "Desktop executable RFC 3161 timestamp is missing" }',
  ].join('; ')
  const result = spawnSync('powershell.exe', ['-NoProfile', '-NonInteractive', '-Command', script], {
    encoding: 'utf8',
    env: {
      ...process.env,
      DSH_VERIFY_EXECUTABLE: executable,
      DSH_WINDOWS_EXPECTED_SIGNER_THUMBPRINT: expectedThumbprint,
    },
  })
  if (result.status !== 0) {
...
function requiredReleaseIdentity(name: string): string {
  const value = process.env[name]
  if (value === undefined || value.trim() === '') {
    throw new Error(`desktop-artifact: 签名验证缺少 ${name}`)
  }
  return value.trim()
}
...
const buildInfo = JSON.parse(asarModule.extractFile(asar, 'build-info.json').toString('utf8')) as {
  version?: unknown
}
if (typeof buildInfo.version !== 'string') throw new Error('desktop-artifact: build-info 缺少产品版本')
const buildSequenceText = process.env.DSH_DESKTOP_BUILD_SEQUENCE ?? '1'
if (!/^\d+$/.test(buildSequenceText)) throw new Error('desktop-artifact: 原生构建序列无效')
const nativeVersions = desktopNativeVersions(buildInfo.version, Number(buildSequenceText))
...
verifyMacBundle(target.app, asar, asarModule, nativeVersions)
