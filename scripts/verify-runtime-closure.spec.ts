...
describe('verifyRuntimeClosure', () => {
  it('requires only plugins active for a Linux or macOS target', async () => {
    const root = fixture({
      ...
      'packages/bundle/gui-app/agent-presets/standard/agent.cordis.yml': `
- id: tools
  name: cordis:group
  group: true
  config:
...
`,
  ...
  it('fails when no shipped preset is discovered', async () => {
    ...
    expect(result.failures).toEqual([
      'no agent presets matched packages/bundle/gui-app/agent-presets/*/agent.cordis.yml',
    ])
  })
  ...
  it('retains the required workspace-peer closure check', async () => {
    const root = fixture({
      ...
      'python/sdk-runtime/platforms.json': platforms,
      'packages/bundle/gui-app/agent-presets/minimal/agent.cordis.yml': '[]\n',
    })
